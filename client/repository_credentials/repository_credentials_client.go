...
// New creates a new repository credentials API client.
func New(transport runtime.ClientTransport, formats strfmt.Registry) *Client {
	return &Client{transport: transport, formats: formats}
}

/*
Client for repository credentials API
*/
type Client struct {
...
func (a *Client) AddRepository(params *AddRepositoryParams, authInfo runtime.ClientAuthInfoWriter) (*AddRepositoryOK, error) {
	...
	success, ok := result.(*AddRepositoryOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for add_repository: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
